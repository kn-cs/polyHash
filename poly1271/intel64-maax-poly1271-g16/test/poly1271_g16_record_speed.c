...
int main() {
	...
	FILE *fpin,*fpout;
    	...
    	// #define SAGE
    	#ifdef SAGE     	
    	c = system("cd ./test_files; sage poly1271_speed_record_test_cases.sage; cd ..");
    	remove("./test_files/poly1271_speed_record_test_cases.sage.py");    	
    	#endif
    	
	format_key(k,KEY_LENGTH,"./test_files/poly1271_speed_record_test_cases_keyfile.txt");
	q = (uint64 *)k; kp[0] = q[0]; kp[1] = q[1];	
    	
   	fpin = fopen("./test_files/poly1271_speed_record_test_cases.txt","r");
    	if (fpin == NULL) {
	...
	fpout = fopen("./test_files/poly1271_g16_speed_records.txt","w");
    	if (fpout == NULL) {
