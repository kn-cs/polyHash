...
void poly1305(uchar8 *h,uchar8 *m,uint64 *k,uint64 l) {
	...
	/* if there is a partial block fill the remainder bytes of the block by 0 and
	 * increase #blocks 
	 */	 
	if (r > 0) {
	 ...
	 *     passing this in purusit of optimizing the
	 *     computation for formatted messages upto 129 bits   
	 */	 
	poly1305maax_precomputed_keypowers((uint64 *)h,(uint64 *)m,k,s,f,n);
}
